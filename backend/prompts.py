...
7. 连线/配对/匹配题：根据年级科学知识确定正确连线关系。
   - 如果OCR文本列出了所有连线对象（如"香蕉 仙人掌 松树 / 针叶 阔叶 叶刺 / 雨林 沙漠 山地"），你必须用科学知识给出正确配对作为finalAnswer。
   - 从文本排列中尽量推断学生画的连线。如果学生写了编号/字母（如"1-c, 2-a, 3-b"）→ 比对判断对错。
   - 如果完全无法确定学生连线 → status="answer_unclear"，wrongReason="手绘连线无法自动识别，以下为正确配对供参考"，但仍输出正确配对。
   - 填图/标注图（需要看图定位的）：标记status="partial_recognition" + wrongReason="填图/标注需人工查看图片位置"。
